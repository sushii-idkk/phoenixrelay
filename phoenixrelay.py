...
# This script is originally written by ruped24! (https://github.com/ruped24/toriptables3)
# I added more features like custom exit nodes and more ip details!
# There is also no LICENSE notice!
# I recreated the code under the "GNU General Public License v3.0" license!
# Contact me if you want me to remove it! contact@phoenixthrush.com
# ============================================================================
...
''' % (basename(__file__), self.trans_port, self.virtual_net,
  ...
  def get_ip(self):
    call("clear")
    print(" {0}".format(
        "\n [\033[92m*\033[0m] Getting details, please wait..."))
    retries = 0
    my_public_ip = None
    while retries < 12 and not my_public_ip:
      ...
      try:
        ...
        my_public_tor = load(urlopen('https://check.torproject.org/api/ip'))['IsTor']
        my_public_city = load(urlopen('https://ipinfo.io'))['city']
        my_public_region = load(urlopen('https://ipinfo.io'))['region']
        my_public_country = load(urlopen('https://ipinfo.io'))['country']
        my_public_org = load(urlopen('https://ipinfo.io'))['org']
        my_public_postal = load(urlopen('https://ipinfo.io'))['postal']
        my_public_timezone = load(urlopen('https://ipinfo.io'))['timezone']
        
      except URLError:
        sleep(5)
        print(" [\033[93m?\033[0m] Still waiting for details...\n")
      except ValueError:
        break
    pass
    if not my_public_ip:
      my_public_ip = getoutput('curl duckduckgo.com')
    if not my_public_ip:
      exit(" \033[91m[!]\033[0m Can't get details! (no internet connection?)")
    
    print(" {0}".format("[\033[92m+\033[0m] You are using tor: \033[92m%s\033[0m" % my_public_tor)) 
    print("\n {0}".format("[\033[92m+\033[0m] Your IP is: \033[92m%s\033[0m" % my_public_ip))
    print(" {0}".format("[\033[92m+\033[0m] Your city is: \033[92m%s\033[0m" % my_public_city))
    print(" {0}".format("[\033[92m+\033[0m] Your region is: \033[92m%s\033[0m" % my_public_region))
    print(" {0}".format("[\033[92m+\033[0m] Your country is: \033[92m%s\033[0m" % my_public_country))
    print(" {0}".format("[\033[92m+\033[0m] Your ISP is: \033[92m%s\033[0m" % my_public_org))
    print(" {0}".format("[\033[92m+\033[0m] Your postal is: \033[92m%s\033[0m" % my_public_postal))
    print(" {0}".format("[\033[92m+\033[0m] Your timezone is: \033[92m%s\033[0m" % my_public_timezone))
    print()
...
if __name__ == '__main__':
  ...
  parser.add_argument('-i',
                      '--ip',
                      action='store_true',
                      help='This option will output details about the current public IP address')
